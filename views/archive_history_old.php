...
<head>
    ...
    <style>
        ...
        * {
        ...
        .archive-table .description-column {
            min-width: 300px;
            white-space: normal;
        }
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            // Load archived records from API
            function loadArchivedRecords(page = 1) {
                const search = $('#searchArchive').val() || '';
                
                $.ajax({
                    url: API_BASE_URL + '?action=getArchivedRecords&page=' + page + '&itemsPerPage=10&search=' + encodeURIComponent(search),
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response && response.success) {
                            archiveData = response.data.archives;
                            currentArchivePage = response.data.current_page;
                            renderArchiveTable();
                            updateArchivePagination(response.data);
                        } else {
                            console.error('Error loading archived records:', response.message);
                            $('#archiveTableBody').html('<tr><td colspan="4" class="text-center">Error loading archived records</td></tr>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('AJAX Error:', status, error);
                        $('#archiveTableBody').html('<tr><td colspan="4" class="text-center">Failed to load archived records</td></tr>');
                    }
                });
            }
            ...
            function renderArchiveTable() {
                ...
                if (!archiveData || archiveData.length === 0) {
                    $tbody.html('<tr><td colspan="4" class="text-center">No archived records found</td></tr>');
                    return;
                }
            ...
            function createArchiveRow(archive) {
                    ...
                    tableHtml = `
                        <tr>
                            <td colspan="3" class="text-center py-4">
                                <span class="material-icons d-block mb-2" style="font-size: 48px; color: #dee2e6;">inbox</span>
                    ...
                    currentItems.forEach(item => {
                        tableHtml += `
                            <tr data-type="${item.type}">
                                <td class="id-column"><strong>${item.id}</strong></td>
                                <td class="type-column"><span class="badge badge-${item.type} archive-badge">${capitalizeFirst(item.type)}</span></td>
                ...
                $('#archiveTableBody').html(tableHtml);

                // Update pagination info
                const startItem = filteredArchiveData.length > 0 ? startIndex + 1 : 0;
                const endItem = Math.min(startIndex + itemsPerPage, filteredArchiveData.length);
                $('#archivePaginationInfo').html(`
                    Showing <span class="font-weight-bold">${startItem}</span> to <span class="font-weight-bold">${endItem}</span> of <span class="font-weight-bold">${filteredArchiveData.length}</span> entries
                `);

                // Render pagination
                renderPagination('archivePagination', currentArchivePage, totalPages, 'archive');
            }
            ...
            function renderPagination(elementId, currentPage, totalPages, type) {
                ...
                // Previous button
                if (currentPage > 1) {
                    paginationHtml += `<li class="page-item"><a class="page-link ${type}-page-link" href="#" data-page="${currentPage - 1}">Previous</a></li>`;
                } else {
                ...
                if (startPage > 1) {
                    paginationHtml += `<li class="page-item"><a class="page-link ${type}-page-link" href="#" data-page="1">1</a></li>`;
                    if (startPage > 2) {
                ...
                for (let i = startPage; i <= endPage; i++) {
                    if (i === currentPage) {
                        ...
                        paginationHtml += `<li class="page-item"><a class="page-link ${type}-page-link" href="#" data-page="${i}">${i}</a></li>`;
                    }
                }
                ...
                if (endPage < totalPages) {
                    ...
                    paginationHtml += `<li class="page-item"><a class="page-link ${type}-page-link" href="#" data-page="${totalPages}">${totalPages}</a></li>`;
                }
                
                // Next button
                if (currentPage < totalPages) {
                    paginationHtml += `<li class="page-item"><a class="page-link ${type}-page-link" href="#" data-page="${currentPage + 1}">Next</a></li>`;
                } else {
            ...
            function getIconForType(type, action) {
